...
import { Row, Col, Container, ListGroup, ListGroupItem, Badge } from "reactstrap";
...
class Stats extends Component {
  state = {
    ...
    categories: ["re-use", "green_action", "food", "lifestyle", "transportation"],
    transportation: [],
    green_action: [],
    food: [],
    lifestyle: [],
  ...
  loadMonthSiteTotal = () => {
    axios
        .get("api/logs/monthSiteTotal/" + this.state.displayMonth + "/" + this.state.displayYear)
        .then( res => {
  ...
  loadCatHighScores = () => {
    ...
    for (let i=0; i< this.state.categories.length; i++) {
      ...
      axios
        .get("/api/logs/monthCatStatsTotal/" + this.state.categories[i] + "/" + this.state.displayMonth + "/" + this.state.displayYear )
        .then ( res => {
          var tempCatHolder = []
          
          tempCatHolder= this.state.categories[i]

          console.log ("Results for " + this.state.categories[i] + " " + res.data)
  ...
  loadHS = () => {
    ...
    axios
      .get("api/logs/monthUserTotal/" + this.state.displayMonth + "/" + this.state.displayYear)
      .then(res => {
    ...
    return (
      <Container>
        ...
        <Row className="justify-content-md-center">
          <div>
            <i className="fa fa-caret-left fa-2x mr-3" onClick={this.monthBack}></i>
          </div>
          <div>
            <h4 className="stats-h4">{moment(this.state.displayMonth).format("MMMM") + " " + this.state.displayYear}</h4>
          </div>
          <div>
            <i className="fa fa-caret-right fa-2x ml-3" aria-hidden="true" onClick={this.monthForward}></i>
          </div>
          ...
          <Row className="justify-content-center">
            ...
            <ListGroup className="text-left">
              {this.state.food.map((item, index) => {
                return (<ListGroupItem key={"food"+ index}className="list-border">
                  <div>{index +1}. {item._id} : <Badge pill className="list-pill">{item.count} pts</Badge></div>
          ...
          <Row className="justify-content-center">
            ...
            <ListGroup className="text-left">
              {this.state.green_action.map((item, index) => {
                return (<ListGroupItem key ={"greenaction" + index}className="list-border">
                  <div>{index +1}. {item._id} : <Badge pill className="list-pill"> {item.count} pts</Badge></div>
          ...
          <Row className="justify-content-center">
            ...
            <ListGroup className="text-left">
              {this.state.lifestyle.map((item, index) => {
                return (<ListGroupItem key={"lifestyle"+ index}className="list-border">
                  <div>{index +1}. {item._id} : <Badge pill className="list-pill"> {item.count} pts</Badge></div>
          ...
          <Row className="justify-content-center">
            ...
            <ListGroup className="text-left">
              {this.state["re-use"].map((item, index) => {
                return (<ListGroupItem key={"reuse"+ index}className="list-border">
                  <div>{index +1}. {item._id} : <Badge pill className="list-pill">{item.count} pts</Badge></div>
          ...
          <Row className="justify-content-center">
            ...
            <ListGroup className="text-left">
              {this.state.transportation.map((item, index) => {
                return (<ListGroupItem key={"trans"+ index}className="list-border">
                  <div>{index +1}. {item._id} : <Badge pill className="list-pill"> {item.count} pts </Badge></div>
